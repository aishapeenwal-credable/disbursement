...
# Extract fields using Together.ai LLM
...
"""
    ...
    for field in FIELDS:
        ...
        elif field == "Invoice date (YYYY-MM-DD)" and "Invoice date" in raw_data:
            parsed[field] = raw_data["Invoice date"]
        else:
            parsed[field] = "Not Extracted"

    return parsed

# Main API: OCR and Match
...
async def ocr_and_match(
    ...
    try:
        ...
        if not id_col:
            raise HTTPException(status_code=400, detail=f"'Invoice ID' column not found in Excel. Found: {original_cols}")

        df["invoice_id_temp_copy"] = df[id_col]
        df = df.fillna("")
        df = df.set_index(id_col)
        excel_data = {str(k).strip().lower(): v for k, v in df.to_dict(orient="index").items()}
        logger.info("📄 Available invoice IDs: %s", list(excel_data.keys())[:5])
    ...
    for doc in documents:
        ...
        excel_row["invoice id"] = invoice_id  # restore
        ...
        for field in FIELDS:
            ...
            norm_excel = normalize_number(excel_val) if any(x in field.lower() for x in ["amount", "gst", "total"]) else excel_val
            norm_ocr = normalize_number(ocr_val) if any(x in field.lower() for x in ["amount", "gst", "total"]) else ocr_val
